...
/** Retorna quadro do guild: { channelId, messageId }. */
...
async function handleBacklogButton(interaction, client) {
    ...
    try {
        const ch = await client.channels.fetch(fullActivity.channelId).catch(() => null);
        const msg = ch ? await ch.messages.fetch(fullActivity.messageId).catch(() => null) : null;
        if (msg) {
